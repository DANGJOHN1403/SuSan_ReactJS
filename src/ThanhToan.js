...
const ThanhToan = ()  => {
    ...
    const submitDuLieu = () => { 
        if (cart.length <= 0) {alert("Giỏ hàng đang trống! Mời bạn hãy chọn sản phẩm nhé "); return;}
    ...
    const luuchitietdonhang = async (id_dh, sp) => {
        ...
        let obj = { 
            id_dh: id_dh, id_sp: sp.id,  ten_sp: sp.ten_sp, gia_mua: sp.gia, 
            sku: sp.sku, so_luong: sp.so_luong
        }
