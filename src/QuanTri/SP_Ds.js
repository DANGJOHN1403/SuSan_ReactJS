   ...
   useEffect ( () => {
    ...
    useEffect(() => {
     ...
     function xoaSP (id) {
        ...
        .then( res => res.json()).then( data  => ganNapLai(naplai+1))
    }
    function locSP(event){ let id_loai = event.target.value; ganidloai(id_loai) }
    return (
      <div id="adminspList">
        <h5 className='sp'>
          <b>Hình</b> <b>Tên SP / SKU 
            <select onChange={ e => locSP(e) } className="float-end shadow-none fs-6 p-1 my-0 me-2">
            <option value="0">Lọc theo loại</option>
            {listLoai.map( (loai, index) => <option key={index} value={loai.id}> {loai.ten_loai}</option>)}
            </select>
          </b> 
          <b>Thông tin </b>  
        ...
        <div className='sp' key={index}>
            ...
            <div>
              <p>Tên SP: <b>{sp.ten_sp}</b> </p> <p>SKU: <b>{sp.sku}</b> </p>          
              <p>Loại: {laytenloai(sp.id_loai) } </p> <p>Tags: {sp.tags} </p>
            </div>          
            <div>
            ...
            <div>
              ...
              <Link to={"/admin/sp_sua/"+sp.id} className='btn btn-primary d-block m-2' >Sửa</Link>
            </div>
        </div>
        ))}
